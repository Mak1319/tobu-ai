...
- **AG-UI** (`api/agui.py`, `api/agui_server.py`) -- turns a LangGraph
  `.stream(...)` run into AG-UI events (see https://docs.ag-ui.com/concepts/events)
  and exposes them over FastAPI as a Server-Sent Events stream at
  `POST /api/agui/run`. `tobu-ai-ui` consumes this endpoint via the
  official `@ag-ui/client` `HttpAgent`. Human-in-the-loop pauses are
  surfaced as `CUSTOM(on_interrupt)` events; the next `RunAgentInput`
  carrying a `resume` payload forwards it as `Command(resume=...)`
  against the same `thread_id`, so chat and voice share one resume
  story.
...
# Bring up the AG-UI HTTP server (consumed by tobu-ai-ui)
...
To start the AG-UI HTTP server (the endpoint `tobu-ai-ui` calls) instead of
the voice worker, override the container command:

```sh
docker compose up -d livekit-agent
docker compose exec livekit-agent python -m main serve --host 0.0.0.0 --port 8000
```

Or run it as a separate service in your compose file with
`command: ["python", "-m", "main", "serve", "--host", "0.0.0.0", "--port", "8000"]`.
...
configuration only governs which LiveKit plugin decides _when_ to
